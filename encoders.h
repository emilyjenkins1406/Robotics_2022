...
#define CPR 358.3      // 29.86 * 12 - gear ratio times counts per rotation on the motor shaft
#define WHEEL_DIAMETER 32 // Wheel diameter in mm

// Volatile Global variables used by Encoder ISR.
// used by encoder to count the rotation left
volatile long count_l; // Left count
volatile byte state_l;
volatile long count_r; // Right count
volatile byte state_r;

enum Motor {left, right};

float circumference = PI * WHEEL_DIAMETER; // Circumference in millimeters

float convertCountToRotations(volatile long count) {
    return count / CPR;
}

float convertRotationsToMillimeters(float rotations) {
    return rotations * circumference;
}

float convertRotationsToMeters(float rotations) {
    return rotations * circumference / 1000;
}

float convertCountToMillimeters(volatile long count) {
    return convertRotationsToMillimeters(convertCountToRotations(count));
}

float convertCountToMeters(volatile long count) {
    return convertRotationsToMeters(convertCountToRotations(count));
}

// Returns the number of rotations performed by the motor
float getRotations(int motor) {
    if (motor == left) {
        return count_l / CPR;
    } else if (motor == right) {
        return count_r / CPR;
...
float getDistance(int motor) {
    float rot = getRotations(motor);
...
ISR(INT6_vect)
{
    ...
    if (state_r == 1 or state_r == 7 or state_r == 8 or state_r == 14)
    {
        count_r -= 1;
    }
    if (state_r == 2 or state_r == 4 or state_r == 11 or state_r == 13)
    {
        count_r += 1;
    }

    //Serial.println((int)state_r);
...
ISR(PCINT0_vect)
{
    ...
    // Complete this if statement as necessary.
    // Refer to the labsheet.
    if (state_l == 1 or state_l == 7 or state_l == 8 or state_l == 14)
    {
        count_l -= 1;
    }
    if (state_l == 2 or state_l == 4 or state_l == 11 or state_l == 13)
    {
        count_l += 1;
    }
